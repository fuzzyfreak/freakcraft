...
DESTFULL="/home/minecraft/mnt/backup/freakcraft-backups/FULLBKPS"
...
tar --exclude='temp' --exclude='back-script.sh' --exclude='*_MYST*' --exclude='*.tar.gz' -zpcvf $DESTFULL/freakcraft-hour0.tar.gz -C $TDESTFULL .

rm -rf $TDESTFULL/ftbserver

#tar -cpvzf $DEST/freakcraft-hour0.tar.gz -C ftbserver
#zip -r $DEST/freakcraft-hour0.zip $TDESTFULL

screen -S minecraft -X stuff "save-on $(printf '\r')"
screen -S minecraft -X stuff "say Save/backup operations complete. $(printf '\r')"
